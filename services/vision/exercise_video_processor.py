...
from detectors.deadlift import DeadliftDetector
from detectors.tricep_extension import TricepExtensionDetector
...
class VideoProcessorClass(VideoProcessorBase):
    def __init__(self):
        ...
        self._detectors = {
            ...
            "Tricep Extension" : TricepExtensionDetector(),
    ...
    def _draw_overlays(self, img, metrics, ex_type):
        ...
        elif ex_type == "Tricep Extension":
            self._draw_tricep_extension_overlays(img, metrics)
        elif ex_type == "Lateral Raises":
            self._draw_lateral_raises_overlays(img, metrics)
        elif ex_type == "Bench Press":
            self._draw_bench_press_overlays(img, metrics)
    ...
    def _draw_deadlift_overlays(self, img, metrics):
        ...
        cv2.putText(
            img,
            f"BACK: {metrics['back_angle']} | HIP: {metrics['hip_angle']} | KNEE: {metrics['knee_angle']}",
            (20, h - 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            1,
            (0, 255, 0),
    ...
    def _draw_tricep_extension_overlays(self, img, metrics):
        h, _ = img.shape[:2]

        cv2.putText(
            img,
            f"ELBOW: {metrics['elbow_angle']} | STATUS: {metrics['elbow_status']}",
            (20, h - 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            1,
            (0, 255, 0),
    ...
    def _draw_lateral_raise_overlays(self, img, metrics):
        h, _ = img.shape[:2]

        cv2.putText(
            img,
            f"SHOULDER: {metrics['shoulder_angle']} | STATUS: {metrics['raise_status']}",
            (20, h - 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            1,
            (0, 255, 0),
    ...
    def _draw_bench_press_overlays(self, img, metrics):
        h, _ = img.shape[:2]

        cv2.putText(
            img,
            f"ELBOW: {metrics['elbow_angle']} | SHOULDER: {metrics['shoulder_angle']} | STATUS: {metrics['press_status']}",
            (20, h - 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            1,
            (0, 255, 0),
