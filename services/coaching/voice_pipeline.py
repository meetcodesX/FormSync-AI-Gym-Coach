...
class VoicePipeline:
    ...
    def _find_form_issue(self, event, exercise, metrics):
        if "issue" in metrics:
        ...
        if exercise == "Squats":
            ...
            if isinstance(back_angle, (int, float)) and back_angle < 150:
                return "Keep your back straight during the deadlift."

            if hip_status == "GOOD POSITION":
                return "Good hip position — keep your core tight."

        elif exercise == "Triceps Extension":
            elbow_angle = metrics.get("elbow_angle", "")
            elbow_status = metrics.get("elbow_status", "")

            if elbow_status == "BENT":
                return "Keep your elbows stable and close to your body."

            if elbow_status == "EXTENDED":
                return "Good extension — control the movement back down."

        elif exercise == "Lateral Raises":
            shoulder_angle = metrics.get("shoulder_angle", "")
            raise_status = metrics.get("raise_status", "")

            if raise_status == "GOOD HEIGHT":
                return "Good height — keep your movement controlled."

            if isinstance(shoulder_angle, (int, float)) and shoulder_angle > 90:
                return "Don't raise your arms too high — keep the movement controlled."

        elif exercise == "Bench Press":
            elbow_angle = metrics.get("elbow_angle", "")
            press_status = metrics.get("press_status", "")

            if press_status == "BOTTOM POSITION":
                return "Keep your wrists stable and press with control."

            if press_status == "LOCKOUT":
                return "Good lockout — control the bar back down."

        return None

    def process_event(self, event, exercise, metrics):
        issue = self._find_form_issue(self,exercise, metrics)
        now = time.time()
